...
class ShadowEnvironmentAdapter:
    ...
    def train_actual_history(self, ms: Microseed, action_id: str, n: int = 12) -> tuple[str, dict]:
        ...
        for i in range(n):
            ...
            out=ms.record_bounded_action_outcome_via_observation_basis(ex["execution"]["execution_id"],observation_capability_id=c.observation_capability_id,observation_obligation=self.obs_obligation(),basis_capability_id=c.observation_basis_id,basis_obligation=self.basis_obligation(),evidence_id=f"E-{self.world.name}-{c.adapter_instance_id}-{action_id}-{i}",capture_id=f"CAP-{self.world.name}-{c.adapter_instance_id}-{action_id}-{i}")
