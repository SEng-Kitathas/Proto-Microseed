    ...
    for raw in sys.stdin:
        try:
            ...
            if op=='reset':
                ...
                visible=context; latent=context; value=0.0; result={'status':'OK'}
            elif op=='apply':
                action=str(msg.get('action_id',''))
                if action=='P1':
                    if visible not in {'s0','r'}: raise ValueError('P1_WRONG_STATE')
                    visible='s1'; value=0.5
                elif action=='P2':
                    if visible!='s1': raise ValueError('P2_WRONG_STATE')
                    visible='s2'; value=1.0
                elif action=='P3':
                    if visible!='s2': raise ValueError('P3_WRONG_STATE')
                    visible='s3'; value=1.5
                elif action=='B':
                    if visible!='s3': raise ValueError('B_WRONG_STATE')
