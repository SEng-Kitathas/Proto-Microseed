...
BASE=ROOT/'reports'/'ms1917_pass06_full_suite_final'/'aggregate_receipt.json'
REPORT=ROOT/'reports'/'ms1917_pass06_terminal_leaf_closure'
...
for gid in terminal_groups:
    r=json.loads((ROOT/'reports'/'ms1917_pass06_full_suite_final'/gid/'receipt.json').read_text(encoding='utf-8'))
...
agg={'schema':'microseed.ms1917.pass06.compatibility-closure.v1','purpose':'close MS1917 terminal timeout leaves from frozen full-suite run by bounded test-function decomposition','base_aggregate_receipt':str(BASE.relative_to(ROOT)),'frozen_source_sha256':start_sha,'source_stable':source_stable,'base_test_file_count':base['test_file_count'],'base_selected_passed_count':base['aggregate_passed_count'],'base_negative_groups':base['negative_groups'],'base_terminal_unknown_groups':terminal_groups,'terminal_files':terminal_files,'expected_leaf_test_count':len(expected_nodes),'leaf_selected_passed_count':leaf_passed,'aggregate_passed_count':base['aggregate_passed_count']+leaf_passed,'exact_test_file_coverage':base['test_file_count'],'leaf_coverage_ok':coverage_ok,'missing_test_nodes':missing,'extra_test_nodes':extra,'duplicate_test_nodes':dup,'negative_groups':[r['group_id'] for r in negatives],'remaining_unknown_groups':[r['group_id'] for r in terminal_unknown],'rounds':rounds,'run_count':len(all_runs),'classification':classification,'completion_marker':'COMPLETE' if classification=='PASS' else 'INCOMPLETE','claim_boundary':'compatibility breadth only; mutation adequacy remains separately owned by final MS1916 source-mutant receipt'}
