...
CONTROL='b022907159d29b8b916b939c1fe92a3dc55520f2'
def health(port):
    try:
...
def write_summary(o):
    ...
    (RUNTIME/'wave1_controller_summary.json').write_text(json.dumps(o,indent=2)+'\n',encoding='utf-8')
...
for arm in ARMS:
    ...
    out=RUNTIME/f'{arm}.stdout.log';err=RUNTIME/f'{arm}.stderr.log'
    ...
    try:
        ...
        child=subprocess.Popen([PY,str(wt/'tools/run_ms_frontier_helix_wave1.py'),'--worktree',str(wt),'--arm',arm,'--max-passes','20','--commit-push'],cwd=wt,stdout=of,stderr=ef)
