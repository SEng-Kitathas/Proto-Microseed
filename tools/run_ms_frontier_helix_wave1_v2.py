...
def write_research_stop(adir:Path,arm:str,records:list[dict[str,Any]],reason:str)->dict[str,Any]:
    audit=csc(records,arm,'RESEARCH_INSTRUMENT_STOP') if records else {'verdict':'REVIEW','terminal_supported':False,'authority':'AUDIT_ONLY_NONE_PROMOTION','errors':[],'warnings':['no admitted passes']}
    stop={'arm':arm,'status':reason,'scientific_closure':False,'passes_completed':len(records),'last_valid_next':records[-1].get('next') if records else None,'csc':audit,'promotion_authority':'NONE'}
    writej(adir/'RESEARCH_STOP.json',stop)
    return stop
...
def run_arm(worktree:Path,arm:str,max_passes:int)->dict[str,Any]:
    ...
    for pnum in range(len(records)+1,max_passes+1):
        ...
        for sem_attempt in range(1,4):
            candidate,meta=chat(PRIMARY,system,attempt_user,max_tokens=1050,temp=.22 if sem_attempt==1 else .08,raw_dir=adir,raw_tag=f'P{pnum:02d}_MODEL_SEM{sem_attempt}')
