...
class Microseed:
    ...
    def derive_and_record_current_native_b3_ordered_composition(self, *, max_records: int = 4096) -> dict[str, Any]:
        """Compose exactly three distinct current grounded referent leaves in token-evidence order.

        This is a hard-bounded arity-3 operational composition owner, not a generic N-ary
        facility and not recursive flattening. The caller supplies no operands, order,
        association ids, referent identities, grouping, or output id.
        """
        base={
            ...
            "authority_gain":"NONE","arity":3,
        }
        bound=int(max_records)
        if bound<=0:
            return {**base,"status":"DEFER_UNKNOWN","reason":"NATIVE_B3_EVIDENCE_SCAN_BUDGET_REQUIRED"}
        total=self.evidence.count()
        if total>bound:
            return {**base,"status":"SEARCH_BUDGET_EXHAUSTED_NOT_SATURATED",
                    "reason":"NATIVE_B3_EVIDENCE_HISTORY_EXCEEDS_SCAN_BUDGET",
                    "total_records":total,"max_records":bound}
        boot=self._current_runtime_boot_seq()
        if boot<0:
            return {**base,"status":"DEFER_UNKNOWN","reason":"CURRENT_RUNTIME_BOOT_BOUNDARY_REQUIRED"}
        ...
        if len(set(referent_sigs))!=3:
            return {**base,"status":"DEFER_UNKNOWN","reason":"THREE_DISTINCT_NATIVE_REFERENT_OPERANDS_REQUIRED"}

        content={
            "operator":"ORDERED_EVIDENCE_TUPLE",
            "ordered_operational_referent_signatures":list(referent_sigs),
            "arity":3,"identity_scope":"OPERATIONAL_EQUIVALENCE_CLASS_ONLY",
        }
        composition_digest=action_result_digest(content)
        payload={
            "kind":"OWNED_NATIVE_B3_ORDERED_OPERATIONAL_REFERENCE_COMPOSITION_EVIDENCE",
            "composition_content_digest_sha256":composition_digest,
            "components":components,
            "ordered_operational_referent_signatures":list(referent_sigs),
            "runtime_boot_seq":boot,
            ...
            "ordering_basis":"CURRENT_RUNTIME_OPAQUE_TOKEN_EVIDENCE_APPEND_ORDER",
            "operand_selection_basis":"LATEST_THREE_CURRENT_RUNTIME_OPAQUE_TOKEN_OBSERVATIONS",
            "association_selection_basis":"UNIQUE_CURRENT_QUALIFIED_NATIVE_TOKEN_REFERENT_ASSOCIATION",
            "identity_scope":"OPERATIONAL_EQUIVALENCE_CLASS_ONLY",
            "arity":3,
            "flattening_authority":"NONE","associativity_authority":"NONE",
        ...
        evidence_id="E-NATIVE-B3-COMPOSITION-"+action_result_digest(payload)[:24]
        existing=self.evidence.get(evidence_id)
        if existing is None:
            ref=self.append_evidence(evidence_id,payload,EpistemicStatus.PRESSURE_SUPPORTED,
                                     source="MICROSEED-NATIVE-B3-ORDERED-COMPOSITION")
            evidence_sha=ref.sha256; record_status="COMPOSITION_EVIDENCE_RECORDED"
        else:
            if existing.get("negative") or existing.get("payload")!=payload:
                return {**base,"status":"DEFER_UNKNOWN","reason":"NATIVE_B3_COMPOSITION_EVIDENCE_ID_COLLISION",
                        "composition_evidence_id":evidence_id}
            evidence_sha=str(existing.get("sha256","")); record_status="COMPOSITION_EVIDENCE_ALREADY_PRESENT"
        return {
            **base,"status":"CURRENT_NATIVE_B3_ORDERED_OPERATIONAL_REFERENCE_COMPOSITION_RECORDED",
            "composition_evidence_id":evidence_id,"composition_evidence_sha256":evidence_sha,
            "composition_record_status":record_status,
            "composition_content_digest_sha256":composition_digest,
            "ordered_operational_referent_signatures":tuple(referent_sigs),
    ...
    def _native_structural_segment_b2_child_carriers(
        ...
        """Project one exact CURRENT 2+2 segment state into LEFT/RIGHT grouped child carriers.

        This is content/provenance adaptation only. It does not create historical B2 evidence,
        flatten either side, infer associativity, or grant semantic/EFFECT authority.
        """
        split=int(segment_current["split_index"])
        components=tuple(boundary_current["components"])
        specs=(
        ...
        for side_ordinal,(side,content,digest,source_components) in enumerate(specs):
            ...
            if (content.get("operator")!="ORDERED_EVIDENCE_TUPLE"
                    or int(content.get("arity",-1))!=2
                    or len(ordered)!=2
                    or len(set(ordered))!=2
                    or len(source_components)!=2):
                return {
                    "status":"DEFER_UNKNOWN",
                    "reason":"SEGMENT_STATE_SIDE_NOT_B2_COMPATIBLE_FOR_RECURSIVE_REUSE",
                    "side":side,"side_arity":int(content.get("arity",-1)),
                }
            ...
            if expected!=str(digest):
                return {"status":"DEFER_UNKNOWN","reason":"SEGMENT_STATE_SIDE_CONTENT_DIGEST_MISMATCH","side":side}
            projected=[]
            for local_ordinal,(component,referent_sig) in enumerate(zip(source_components,ordered)):
                if not isinstance(component,dict) or str(component.get("operational_referent_signature_sha256",""))!=referent_sig:
                    return {"status":"DEFER_UNKNOWN","reason":"SEGMENT_STATE_SIDE_SOURCE_COMPONENT_MISMATCH","side":side}
                projected.append({
            ...
            children.append({
                "ordinal":side_ordinal,"source_side":side,
                "composition_content_digest_sha256":expected,
                "ordered_operational_referent_signatures":list(ordered),
                "validated_components":projected,
        ...
        if len({c["composition_content_digest_sha256"] for c in children})!=2:
            return {"status":"DEFER_UNKNOWN","reason":"TWO_DISTINCT_SEGMENT_SIDE_CHILD_CONTENTS_REQUIRED"}
        return {"status":"CURRENT_B2_COMPATIBLE_STRUCTURAL_SEGMENT_CHILDREN", "children":tuple(children)}
    ...
    def _validate_current_native_structural_segment_b2_recursive_composition_state(
        ...
        base={
            ...
            "recursive_depth_limit":1,"admitted_child_arity":2,
        }
        if row.get("negative"):
            return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_RECURSIVE_STATE_MUST_BE_POSITIVE"}
        payload=row.get("payload") or {}
        if payload.get("kind")!="OWNED_NATIVE_STRUCTURAL_SEGMENT_B2_RECURSIVE_COMPOSITION_STATE":
            return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_RECURSIVE_STATE_KIND_REQUIRED"}
        if int(payload.get("runtime_boot_seq",-1))!=boot:
            return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_RECURSIVE_STATE_CURRENT_BOOT_REQUIRED"}
        if (payload.get("operator_owner")!="MICROSEED_NATIVE_STRUCTURAL_SEGMENT_B2_RECURSIVE_COMPOSITION"
                ...
                or payload.get("selection_basis")!="OLDEST_CURRENT_UNCONSUMED_B2_COMPATIBLE_STRUCTURAL_SEGMENT_STATE_BY_EVIDENCE_APPEND_ORDER"
                or payload.get("grouping_basis")!="EXACT_SEGMENT_STATE_LEFT_RIGHT_BOUNDARY_PRESERVED"):
            return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_RECURSIVE_STATE_OWNER_OR_SHAPE_MISMATCH"}
        for k in (
            ...
            if payload.get(k)!="NONE":
                return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_RECURSIVE_STATE_AUTHORITY_OVERCLAIM","field":k}
        expected_id="E-NATIVE-STRUCTURAL-SEGMENT-B2-RECURSIVE-"+action_result_digest(payload)[:24]
        if str(row.get("evidence_id",""))!=expected_id:
            return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_RECURSIVE_STATE_EVIDENCE_ID_NOT_DERIVED_FROM_CONTENT",
                    "expected_evidence_id":expected_id}
        segment_ref=payload.get("segment_state_evidence_ref") or ()
        if len(segment_ref)!=2:
            return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_RECURSIVE_STATE_SEGMENT_REF_REQUIRED"}
        segment_row=self.evidence.get(str(segment_ref[0]))
        if segment_row is None or str(segment_row.get("sha256",""))!=str(segment_ref[1]):
            return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_RECURSIVE_STATE_SEGMENT_REF_NOT_EXACT"}
        ...
        if boundary_row is None or str(boundary_row.get("sha256",""))!=str(segment_current["boundary_evidence_sha256"]):
            return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_RECURSIVE_STATE_BOUNDARY_REF_NOT_EXACT"}
        boundary_current=self._validate_current_native_structural_boundary_witness(boundary_row,rows=rows,boot=boot)
        if boundary_current.get("status")!="CURRENT_NATIVE_STRUCTURAL_BOUNDARY_WITNESS":
            return {**base,**boundary_current,"status":"DEFER_UNKNOWN"}
        projected=self._native_structural_segment_b2_child_carriers(segment_current,boundary_current)
        if projected.get("status")!="CURRENT_B2_COMPATIBLE_STRUCTURAL_SEGMENT_CHILDREN":
        ...
        if (payload.get("children")!=list(children)
            ...
            return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_RECURSIVE_STATE_CONTENT_OR_CHILD_MISMATCH"}
        return {
            **base,"status":"CURRENT_NATIVE_STRUCTURAL_SEGMENT_B2_RECURSIVE_COMPOSITION_STATE",
    ...
    def derive_and_record_current_native_structural_segment_b2_recursive_composition(
        ...
        """Reuse one CURRENT 2+2 structural segment state as two grouped recursive operands.

        The oldest CURRENT unconsumed B2-compatible segment state is chosen by evidence append
        order. LEFT then RIGHT grouping comes only from the persisted segment state. The result
        is append-only current retrospective composition state whose *content* shares the already-
        earned recursive-B2 identity law; it is not a historical B2 composition event.
        """
        base={
            "selection_basis":"OLDEST_CURRENT_UNCONSUMED_B2_COMPATIBLE_STRUCTURAL_SEGMENT_STATE_BY_EVIDENCE_APPEND_ORDER",
        ...
        bound=int(max_records)
        if bound<=0:
            return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_RECURSIVE_EVIDENCE_SCAN_BUDGET_REQUIRED"}
        total=self.evidence.count()
        if total>bound:
            return {**base,"status":"SEARCH_BUDGET_EXHAUSTED_NOT_SATURATED",
                    "reason":"SEGMENT_RECURSIVE_EVIDENCE_HISTORY_EXCEEDS_SCAN_BUDGET",
                    "total_records":total,"max_records":bound}
        boot=self._current_runtime_boot_seq()
        if boot<0:
            return {**base,"status":"DEFER_UNKNOWN","reason":"CURRENT_RUNTIME_BOOT_BOUNDARY_REQUIRED"}
        ...
        for pos,row in enumerate(rows):
            ...
            if payload.get("kind")=="OWNED_NATIVE_STRUCTURAL_SEGMENT_COMPOSITION_STATE":
                ...
                boundary_current=self._validate_current_native_structural_boundary_witness(boundary_row,rows=rows,boot=boot) if boundary_row is not None else {"status":"DEFER_UNKNOWN","reason":"SEGMENT_RECURSIVE_BOUNDARY_REQUIRED"}
                if boundary_current.get("status")!="CURRENT_NATIVE_STRUCTURAL_BOUNDARY_WITNESS":
                    return {**base,**boundary_current,"status":"DEFER_UNKNOWN"}
                projected=self._native_structural_segment_b2_child_carriers(current,boundary_current)
                segments.append((pos,row,current,projected))
            elif payload.get("kind")=="OWNED_NATIVE_STRUCTURAL_SEGMENT_B2_RECURSIVE_COMPOSITION_STATE":
                current=self._validate_current_native_structural_segment_b2_recursive_composition_state(row,rows=rows,boot=boot)
                if current.get("status")!="CURRENT_NATIVE_STRUCTURAL_SEGMENT_B2_RECURSIVE_COMPOSITION_STATE":
                    return {**base,**current,"status":"DEFER_UNKNOWN"}
        ...
        for _pos,_row,current in parents:
            ...
            if sid in consumed:
                return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_RECURSIVE_SEGMENT_STATE_REPLAY_DETECTED",
                        "segment_state_evidence_id":sid}
            consumed.append(sid)
        consumed_set=set(consumed)
        compatible=[]; skipped=[]
        for item in segments:
            _pos,row,current,projected=item
            sid=str(row["evidence_id"])
            ...
            if projected.get("status")=="CURRENT_B2_COMPATIBLE_STRUCTURAL_SEGMENT_CHILDREN":
                ...
                skipped.append({
                    "segment_state_evidence_id":sid,
                    "reason":projected.get("reason","SEGMENT_STATE_NOT_B2_COMPATIBLE"),
                    "side":projected.get("side"),"side_arity":projected.get("side_arity"),
                })
        if not compatible:
            if parents and not skipped:
                _pos,_row,current=parents[-1]
                return {**base,**current,
                        "status":"CURRENT_NATIVE_STRUCTURAL_SEGMENT_B2_RECURSIVE_COMPOSITION_STATE_ALREADY_PRESENT",
                        "composition_state_record_status":"SEGMENT_RECURSIVE_STATE_ALREADY_PRESENT",
                        ...
                        "caller_supplied_output_evidence_id":"NO"}
            return {**base,"status":"DEFER_UNKNOWN",
                    "reason":"CURRENT_UNCONSUMED_B2_COMPATIBLE_STRUCTURAL_SEGMENT_STATE_REQUIRED",
        ...
        payload={
            "kind":"OWNED_NATIVE_STRUCTURAL_SEGMENT_B2_RECURSIVE_COMPOSITION_STATE",
            "runtime_boot_seq":boot,
            "operator_owner":"MICROSEED_NATIVE_STRUCTURAL_SEGMENT_B2_RECURSIVE_COMPOSITION",
            ...
            "selection_basis":base["selection_basis"],
            "grouping_basis":base["grouping_basis"],
        ...
        evidence_id="E-NATIVE-STRUCTURAL-SEGMENT-B2-RECURSIVE-"+action_result_digest(payload)[:24]
        existing=self.evidence.get(evidence_id)
        if existing is None:
            ref=self.append_evidence(evidence_id,payload,EpistemicStatus.PRESSURE_SUPPORTED,
                                     source="MICROSEED-NATIVE-STRUCTURAL-SEGMENT-B2-RECURSIVE-COMPOSITION")
            evidence_sha=ref.sha256; record_status="SEGMENT_RECURSIVE_STATE_RECORDED"
        else:
            if existing.get("negative") or existing.get("payload")!=payload:
                return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_RECURSIVE_STATE_EVIDENCE_ID_COLLISION",
                        "composition_state_evidence_id":evidence_id}
            evidence_sha=str(existing.get("sha256","")); record_status="SEGMENT_RECURSIVE_STATE_ALREADY_PRESENT"
        return {
            **base,"status":"CURRENT_NATIVE_STRUCTURAL_SEGMENT_B2_RECURSIVE_COMPOSITION_STATE_RECORDED",
    ...
    def _native_depth_one_structural_segment_parent_child_carrier(
        ...
        """Normalize one exact CURRENT retrospective depth-one segment parent as one nested child."""
        payload=row.get("payload") or {}
        kind=str(payload.get("kind",""))
        if kind=="OWNED_NATIVE_STRUCTURAL_SEGMENT_B2_RECURSIVE_COMPOSITION_STATE":
            current=self._validate_current_native_structural_segment_b2_recursive_composition_state(row,rows=rows,boot=boot)
            required="CURRENT_NATIVE_STRUCTURAL_SEGMENT_B2_RECURSIVE_COMPOSITION_STATE"
        elif kind=="OWNED_NATIVE_STRUCTURAL_SEGMENT_BOUNDED_RECURSIVE_COMPOSITION_STATE":
            current=self._validate_current_native_structural_segment_bounded_recursive_composition_state(row,rows=rows,boot=boot)
            required="CURRENT_NATIVE_STRUCTURAL_SEGMENT_BOUNDED_RECURSIVE_COMPOSITION_STATE"
        else:
            return {"status":"DEFER_UNKNOWN","reason":"DEPTH_ONE_RETROSPECTIVE_SEGMENT_PARENT_KIND_REQUIRED"}
        if current.get("status")!=required:
            return {**current,"status":"DEFER_UNKNOWN"}
        if int(current.get("composition_depth",-1))!=1 or int(current.get("child_arity",-1))!=2:
            return {"status":"DEFER_UNKNOWN","reason":"EXACT_DEPTH_ONE_TWO_CHILD_SEGMENT_PARENT_REQUIRED","source_parent_kind":kind}
        nested_children=tuple(current.get("children",()))
        nested_child_digests=tuple(str(x) for x in current.get("ordered_child_composition_content_digests",()))
        if len(nested_children)!=2 or len(nested_child_digests)!=2 or len(set(nested_child_digests))!=2:
            return {"status":"DEFER_UNKNOWN","reason":"EXACT_DISTINCT_GROUPED_DEPTH_ONE_SEGMENT_PARENT_REQUIRED","source_parent_kind":kind}
        expected=action_result_digest({
            "operator":"RECURSIVE_ORDERED_EVIDENCE_TUPLE",
            "ordered_child_composition_content_digests":list(nested_child_digests),
            "composition_depth":1,"child_arity":2,
            "identity_scope":"EXACT_GROUPED_OPERATIONAL_COMPOSITION_ONLY",
        })
        if expected!=str(current.get("composition_content_digest_sha256","")):
            return {"status":"DEFER_UNKNOWN","reason":"DEPTH_ONE_SEGMENT_PARENT_CONTENT_DIGEST_MISMATCH","source_parent_kind":kind}
        return {
            "status":"CURRENT_DEPTH_ONE_STRUCTURAL_SEGMENT_PARENT_CHILD",
            "source_parent_kind":kind,
            "source_parent_evidence_ref":[str(row["evidence_id"]),str(row["sha256"])],
            "source_segment_state_evidence_ref":[str(current["segment_state_evidence_id"]),str(current["segment_state_evidence_sha256"])],
            ...
            "nested_child_content_digests":nested_child_digests,
    ...
    def _validate_current_native_structural_segment_depth_two_recursive_composition_state(
        ...
        base={
            "input_parent_depth":1,"composition_depth":2,"child_arity":2,
            "recursive_depth_limit":2,"generic_recursive_closure_authority":"NONE",
            "depth_three_authority":"NONE","historical_event_authority":"NONE",
            "ledger_rewrite_authority":"NONE","flattening_authority":"NONE",
            "associativity_authority":"NONE","semantic_composition_authority":"NONE",
            "grammar_authority":"NONE","effect_authority":"NONE","execution_authority":"NONE",
            "scheduler_authority":"NONE","authority_gain":"NONE",
        ...
        if row.get("negative"):
            return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_DEPTH_TWO_STATE_MUST_BE_POSITIVE"}
        payload=row.get("payload") or {}
        if payload.get("kind")!="OWNED_NATIVE_STRUCTURAL_SEGMENT_DEPTH_TWO_RECURSIVE_COMPOSITION_STATE":
            return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_DEPTH_TWO_STATE_KIND_REQUIRED"}
        if int(payload.get("runtime_boot_seq",-1))!=boot:
            return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_DEPTH_TWO_STATE_CURRENT_BOOT_REQUIRED"}
        if (payload.get("operator_owner")!="MICROSEED_NATIVE_STRUCTURAL_SEGMENT_DEPTH_TWO_RECURSIVE_COMPOSITION"
                or payload.get("composition_operator")!="RECURSIVE_ORDERED_EVIDENCE_TUPLE"
                or int(payload.get("input_parent_depth",-1))!=1
                or int(payload.get("composition_depth",-1))!=2
                or int(payload.get("child_arity",-1))!=2
                or payload.get("selection_basis")!="LATEST_TWO_DISTINCT_CURRENT_DEPTH_ONE_RETROSPECTIVE_SEGMENT_PARENT_CONTENTS_IN_EVIDENCE_ORDER"
                or payload.get("grouping_basis")!="EXACT_DEPTH_ONE_SEGMENT_PARENT_BOUNDARIES_PRESERVED"
                or payload.get("temporality")!="CURRENT_RETROSPECTIVE_DERIVATION_APPENDED_AFTER_DEPTH_ONE_PARENT_STATES"):
            return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_DEPTH_TWO_STATE_OWNER_OR_SHAPE_MISMATCH"}
        for k in (
            "historical_event_authority","ledger_rewrite_authority","flattening_authority",
            "associativity_authority","semantic_composition_authority","grammar_authority",
            "effect_authority","execution_authority","scheduler_authority","authority_gain",
            "generic_recursive_closure_authority","depth_three_authority",
        ):
            if payload.get(k)!="NONE":
                return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_DEPTH_TWO_STATE_AUTHORITY_OVERCLAIM","field":k}
        expected_id="E-NATIVE-STRUCTURAL-SEGMENT-DEPTH-TWO-RECURSIVE-"+action_result_digest(payload)[:24]
        if str(row.get("evidence_id",""))!=expected_id:
            return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_DEPTH_TWO_STATE_EVIDENCE_ID_NOT_DERIVED_FROM_CONTENT","expected_evidence_id":expected_id}
        ...
        if len(payload_children)!=2 or len(child_digests)!=2 or len(set(child_digests))!=2:
            return {**base,"status":"DEFER_UNKNOWN","reason":"TWO_DISTINCT_DEPTH_ONE_PARENT_CHILDREN_REQUIRED"}
        validated=[]
        for ordinal,(child_payload,expected_digest) in enumerate(zip(payload_children,child_digests)):
            if not isinstance(child_payload,dict) or int(child_payload.get("ordinal",-1))!=ordinal:
                return {**base,"status":"DEFER_UNKNOWN","reason":"DEPTH_TWO_CHILD_ORDINAL_MISMATCH"}
            ref=tuple(child_payload.get("source_parent_evidence_ref",()))
            if len(ref)!=2:
                return {**base,"status":"DEFER_UNKNOWN","reason":"DEPTH_TWO_CHILD_PARENT_REF_REQUIRED"}
            parent_row=self.evidence.get(str(ref[0]))
            if parent_row is None or str(parent_row.get("sha256",""))!=str(ref[1]):
                return {**base,"status":"DEFER_UNKNOWN","reason":"DEPTH_TWO_CHILD_PARENT_REF_NOT_EXACT"}
            ...
            if child_payload!=expected_child or str(carrier["composition_content_digest_sha256"])!=expected_digest:
                return {**base,"status":"DEFER_UNKNOWN","reason":"DEPTH_TWO_CHILD_CONTENT_OR_PROVENANCE_MISMATCH"}
        ...
        if str(payload.get("composition_content_digest_sha256",""))!=digest:
            return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_DEPTH_TWO_STATE_CONTENT_DIGEST_MISMATCH"}
        return {
            **base,"status":"CURRENT_NATIVE_STRUCTURAL_SEGMENT_DEPTH_TWO_RECURSIVE_COMPOSITION_STATE",
            "composition_state_evidence_id":str(row["evidence_id"]),
            "composition_state_evidence_sha256":str(row["sha256"]),
            "composition_content_digest_sha256":digest,
            "ordered_child_composition_content_digests":child_digests,
    ...
    def derive_and_record_current_native_structural_segment_depth_two_recursive_composition(
        ...
        """Compose the latest two distinct CURRENT depth-one retrospective segment parents once.

        This owner is intentionally not recursively closed: only the two already-earned depth-one
        retrospective segment-parent kinds are eligible inputs. Its own depth-two output kind is
        validated for tamper/currentness but is never admitted as a child.
        """
        base={
            "selection_basis":"LATEST_TWO_DISTINCT_CURRENT_DEPTH_ONE_RETROSPECTIVE_SEGMENT_PARENT_CONTENTS_IN_EVIDENCE_ORDER",
            "grouping_basis":"EXACT_DEPTH_ONE_SEGMENT_PARENT_BOUNDARIES_PRESERVED",
            "input_parent_depth":1,"composition_depth":2,"child_arity":2,
            "recursive_depth_limit":2,"generic_recursive_closure_authority":"NONE",
            "depth_three_authority":"NONE","historical_event_authority":"NONE",
            "ledger_rewrite_authority":"NONE","flattening_authority":"NONE",
        ...
        bound=int(max_records)
        if bound<=0:
            return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_DEPTH_TWO_EVIDENCE_SCAN_BUDGET_REQUIRED"}
        total=self.evidence.count()
        if total>bound:
            return {**base,"status":"SEARCH_BUDGET_EXHAUSTED_NOT_SATURATED",
                    "reason":"SEGMENT_DEPTH_TWO_EVIDENCE_HISTORY_EXCEEDS_SCAN_BUDGET",
                    "total_records":total,"max_records":bound}
        boot=self._current_runtime_boot_seq()
        if boot<0:
            return {**base,"status":"DEFER_UNKNOWN","reason":"CURRENT_RUNTIME_BOOT_BOUNDARY_REQUIRED"}
        rows=self.evidence.list(); candidates=[]
        accepted={
        ...
        for pos,row in enumerate(rows):
            ...
            if kind in accepted:
                ...
                if carrier.get("status")!="CURRENT_DEPTH_ONE_STRUCTURAL_SEGMENT_PARENT_CHILD":
                    return {**base,**carrier,"status":"DEFER_UNKNOWN"}
                carrier["evidence_list_position"]=pos; candidates.append(carrier)
        ...
        if len(selected_rev)<2:
            return {**base,"status":"DEFER_UNKNOWN",
                    "reason":"TWO_DISTINCT_CURRENT_DEPTH_ONE_SEGMENT_PARENT_CONTENTS_REQUIRED",
                    "current_distinct_parent_count":len(selected_rev)}
        ...
        payload={
            "kind":"OWNED_NATIVE_STRUCTURAL_SEGMENT_DEPTH_TWO_RECURSIVE_COMPOSITION_STATE",
            "runtime_boot_seq":boot,
            "operator_owner":"MICROSEED_NATIVE_STRUCTURAL_SEGMENT_DEPTH_TWO_RECURSIVE_COMPOSITION",
            "composition_content_digest_sha256":digest,
            "composition_operator":"RECURSIVE_ORDERED_EVIDENCE_TUPLE",
            "input_parent_depth":1,"composition_depth":2,"child_arity":2,
            "ordered_child_composition_content_digests":list(child_digests),
            ...
            "selection_basis":base["selection_basis"],"grouping_basis":base["grouping_basis"],
            "identity_scope":"EXACT_GROUPED_OPERATIONAL_COMPOSITION_ONLY",
            "temporality":"CURRENT_RETROSPECTIVE_DERIVATION_APPENDED_AFTER_DEPTH_ONE_PARENT_STATES",
            "generic_recursive_closure_authority":"NONE","depth_three_authority":"NONE",
            "historical_event_authority":"NONE","ledger_rewrite_authority":"NONE",
            "flattening_authority":"NONE","associativity_authority":"NONE",
            "semantic_composition_authority":"NONE","grammar_authority":"NONE",
            "effect_authority":"NONE","execution_authority":"NONE",
        ...
        evidence_id="E-NATIVE-STRUCTURAL-SEGMENT-DEPTH-TWO-RECURSIVE-"+action_result_digest(payload)[:24]
        existing=self.evidence.get(evidence_id)
        if existing is None:
            ref=self.append_evidence(evidence_id,payload,EpistemicStatus.PRESSURE_SUPPORTED,
                                     source="MICROSEED-NATIVE-STRUCTURAL-SEGMENT-DEPTH-TWO-RECURSIVE-COMPOSITION")
            evidence_sha=ref.sha256; record_status="SEGMENT_DEPTH_TWO_STATE_RECORDED"
        else:
            if existing.get("negative") or existing.get("payload")!=payload:
                return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_DEPTH_TWO_STATE_EVIDENCE_ID_COLLISION","composition_state_evidence_id":evidence_id}
            evidence_sha=str(existing.get("sha256","")); record_status="SEGMENT_DEPTH_TWO_STATE_ALREADY_PRESENT"
        return {
            **base,"status":"CURRENT_NATIVE_STRUCTURAL_SEGMENT_DEPTH_TWO_RECURSIVE_COMPOSITION_STATE_RECORDED",
            "composition_state_evidence_id":evidence_id,"composition_state_evidence_sha256":evidence_sha,
            "composition_state_record_status":record_status,
            "composition_content_digest_sha256":digest,
            "ordered_child_composition_content_digests":child_digests,
    ...
    def _native_depth_three_structural_segment_parent_child_carrier(
        ...
        """Normalize one exact CURRENT depth-three retrospective segment parent as one nested child."""
        current=self._validate_current_native_structural_segment_depth_three_recursive_composition_state(row,rows=rows,boot=boot)
        if current.get("status")!="CURRENT_NATIVE_STRUCTURAL_SEGMENT_DEPTH_THREE_RECURSIVE_COMPOSITION_STATE":
            return {**current,"status":"DEFER_UNKNOWN"}
        ancestry=set()
        for child in current.get("children",()):
            ancestry.add(str(child.get("composition_content_digest_sha256","")))
            for d in child.get("nested_child_content_digests",()):
                ancestry.add(str(d))
        ancestry.discard("")
        return {
            "status":"CURRENT_DEPTH_THREE_STRUCTURAL_SEGMENT_PARENT_CHILD",
            "source_parent_kind":"OWNED_NATIVE_STRUCTURAL_SEGMENT_DEPTH_THREE_RECURSIVE_COMPOSITION_STATE",
            "source_parent_evidence_ref":[str(row["evidence_id"]),str(row["sha256"])],
            "composition_content_digest_sha256":str(current["composition_content_digest_sha256"]),
            "composition_depth":3,"child_arity":2,
            "nested_child_content_digests":tuple(str(x) for x in current["ordered_child_composition_content_digests"]),
    ...
    def _validate_current_native_structural_segment_depth_four_recursive_composition_state(
        ...
        base={
            "max_input_parent_depth":3,"composition_depth":4,"child_arity":2,
            "recursive_depth_limit":4,"generic_recursive_closure_authority":"NONE",
            "depth_five_authority":"NONE","historical_event_authority":"NONE",
            "ledger_rewrite_authority":"NONE","flattening_authority":"NONE",
            "associativity_authority":"NONE","semantic_composition_authority":"NONE",
            "grammar_authority":"NONE","effect_authority":"NONE","execution_authority":"NONE",
            "scheduler_authority":"NONE","authority_gain":"NONE",
        ...
        if row.get("negative"):
            return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_DEPTH_FOUR_STATE_MUST_BE_POSITIVE"}
        payload=row.get("payload") or {}
        if payload.get("kind")!="OWNED_NATIVE_STRUCTURAL_SEGMENT_DEPTH_FOUR_RECURSIVE_COMPOSITION_STATE":
            return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_DEPTH_FOUR_STATE_KIND_REQUIRED"}
        if int(payload.get("runtime_boot_seq",-1))!=boot:
            return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_DEPTH_FOUR_STATE_CURRENT_BOOT_REQUIRED"}
        if (payload.get("operator_owner")!="MICROSEED_NATIVE_STRUCTURAL_SEGMENT_DEPTH_FOUR_RECURSIVE_COMPOSITION"
                or payload.get("composition_operator")!="RECURSIVE_ORDERED_EVIDENCE_TUPLE"
                or int(payload.get("max_input_parent_depth",-1))!=3
                or int(payload.get("composition_depth",-1))!=4
                or int(payload.get("child_arity",-1))!=2
                or payload.get("selection_basis")!="LATEST_CURRENT_DEPTH_THREE_PARENT_PLUS_LATEST_CURRENT_EXTERNAL_DEPTH_ONE_PARENT_IN_EVIDENCE_ORDER"
                or payload.get("grouping_basis")!="WHOLE_DEPTH_THREE_AND_DEPTH_ONE_PARENT_BOUNDARIES_PRESERVED"
                or payload.get("temporality")!="CURRENT_RETROSPECTIVE_DERIVATION_APPENDED_AFTER_DEPTH_THREE_AND_EXTERNAL_DEPTH_ONE_PARENT_STATES"):
            return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_DEPTH_FOUR_STATE_OWNER_OR_SHAPE_MISMATCH"}
        for k in (
            "historical_event_authority","ledger_rewrite_authority","flattening_authority",
            "associativity_authority","semantic_composition_authority","grammar_authority",
            "effect_authority","execution_authority","scheduler_authority","authority_gain",
            "generic_recursive_closure_authority","depth_five_authority",
        ):
            if payload.get(k)!="NONE":
                return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_DEPTH_FOUR_STATE_AUTHORITY_OVERCLAIM","field":k}
        expected_id="E-NATIVE-STRUCTURAL-SEGMENT-DEPTH-FOUR-RECURSIVE-"+action_result_digest(payload)[:24]
        if str(row.get("evidence_id",""))!=expected_id:
            return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_DEPTH_FOUR_STATE_EVIDENCE_ID_NOT_DERIVED_FROM_CONTENT","expected_evidence_id":expected_id}
        payload_children=tuple(payload.get("children",()))
        child_digests=tuple(str(x) for x in payload.get("ordered_child_composition_content_digests",()))
        child_depths=tuple(int(x) for x in payload.get("ordered_child_composition_depths",()))
        if len(payload_children)!=2 or len(child_digests)!=2 or len(set(child_digests))!=2 or len(child_depths)!=2 or sorted(child_depths)!=[1,3]:
            return {**base,"status":"DEFER_UNKNOWN","reason":"EXACT_ONE_DEPTH_THREE_AND_ONE_DEPTH_ONE_CHILD_REQUIRED"}
        positions={str(r.get("evidence_id","")):i for i,r in enumerate(rows)}
        validated=[]; evidence_positions=[]; depth_three_current=None; depth_one_current=None
        for ordinal,(child_payload,expected_digest,expected_depth) in enumerate(zip(payload_children,child_digests,child_depths)):
            if not isinstance(child_payload,dict) or int(child_payload.get("ordinal",-1))!=ordinal or int(child_payload.get("composition_depth",-1))!=expected_depth:
                return {**base,"status":"DEFER_UNKNOWN","reason":"DEPTH_FOUR_CHILD_ORDINAL_OR_DEPTH_MISMATCH"}
            ref=tuple(child_payload.get("source_parent_evidence_ref",()))
            if len(ref)!=2:
                return {**base,"status":"DEFER_UNKNOWN","reason":"DEPTH_FOUR_CHILD_PARENT_REF_REQUIRED"}
            parent_row=self.evidence.get(str(ref[0]))
            if parent_row is None or str(parent_row.get("sha256",""))!=str(ref[1]):
                return {**base,"status":"DEFER_UNKNOWN","reason":"DEPTH_FOUR_CHILD_PARENT_REF_NOT_EXACT"}
            evidence_positions.append(int(positions.get(str(ref[0]),-1)))
            if expected_depth==3:
                carrier=self._native_depth_three_structural_segment_parent_child_carrier(parent_row,rows=rows,boot=boot)
                if carrier.get("status")!="CURRENT_DEPTH_THREE_STRUCTURAL_SEGMENT_PARENT_CHILD":
                    return {**base,**carrier,"status":"DEFER_UNKNOWN"}
                expected_child={
                    "ordinal":ordinal,
                    "source_parent_kind":"OWNED_NATIVE_STRUCTURAL_SEGMENT_DEPTH_THREE_RECURSIVE_COMPOSITION_STATE",
                    ...
                    "composition_depth":3,"child_arity":2,
                    "nested_child_content_digests":list(carrier["nested_child_content_digests"]),
                    "full_ancestry_content_digests":list(carrier["full_ancestry_content_digests"]),
                    "retrospective_provenance":"CURRENT_APPEND_ONLY_DEPTH_THREE_SEGMENT_PARENT_STATE",
                    "historical_event_authority":"NONE","flattening_authority":"NONE",
                    "associativity_authority":"NONE","authority_gain":"NONE",
                }
                depth_three_current=carrier
            else:
                carrier=self._native_depth_one_structural_segment_parent_child_carrier(parent_row,rows=rows,boot=boot)
                if carrier.get("status")!="CURRENT_DEPTH_ONE_STRUCTURAL_SEGMENT_PARENT_CHILD":
                    return {**base,**carrier,"status":"DEFER_UNKNOWN"}
                ...
                depth_one_current=carrier
            if child_payload!=expected_child or str(carrier["composition_content_digest_sha256"])!=expected_digest:
                return {**base,"status":"DEFER_UNKNOWN","reason":"DEPTH_FOUR_CHILD_CONTENT_OR_PROVENANCE_MISMATCH"}
            validated.append(carrier)
        if any(x<0 for x in evidence_positions) or evidence_positions!=sorted(evidence_positions):
            return {**base,"status":"DEFER_UNKNOWN","reason":"DEPTH_FOUR_CHILD_ORDER_NOT_EVIDENCE_OWNED"}
        if depth_three_current is None or depth_one_current is None:
            return {**base,"status":"DEFER_UNKNOWN","reason":"EXACT_ONE_DEPTH_THREE_AND_ONE_DEPTH_ONE_CHILD_REQUIRED"}
        ancestry=set(str(x) for x in depth_three_current["full_ancestry_content_digests"])
        ancestry.add(str(depth_three_current["composition_content_digest_sha256"]))
        if str(depth_one_current["composition_content_digest_sha256"]) in ancestry:
            return {**base,"status":"DEFER_UNKNOWN","reason":"DEPTH_FOUR_EXTERNAL_DEPTH_ONE_PARENT_MUST_BE_OUTSIDE_FULL_DEPTH_THREE_ANCESTRY"}
        content={
            "operator":"RECURSIVE_ORDERED_EVIDENCE_TUPLE",
            "ordered_child_composition_content_digests":list(child_digests),
            "ordered_child_composition_depths":list(child_depths),
        ...
        digest=action_result_digest(content)
        if str(payload.get("composition_content_digest_sha256",""))!=digest:
            return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_DEPTH_FOUR_STATE_CONTENT_DIGEST_MISMATCH"}
        return {
            **base,"status":"CURRENT_NATIVE_STRUCTURAL_SEGMENT_DEPTH_FOUR_RECURSIVE_COMPOSITION_STATE",
            "composition_state_evidence_id":str(row["evidence_id"]),
            "composition_state_evidence_sha256":str(row["sha256"]),
            "composition_content_digest_sha256":digest,
            "ordered_child_composition_content_digests":child_digests,
    ...
    def derive_and_record_current_native_structural_segment_depth_four_recursive_composition(
        ...
        """Compose one CURRENT depth-three parent with one external CURRENT depth-one parent once."""
        base={
        ...
        if max_records<=0:
            return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_DEPTH_FOUR_EVIDENCE_SCAN_BUDGET_REQUIRED"}
        total=self.evidence.count()
        if total>max_records:
        ...
        if boot<0:
            return {**base,"status":"DEFER_UNKNOWN","reason":"CURRENT_RUNTIME_BOOT_BOUNDARY_REQUIRED"}
        rows=self.evidence.list(); depth_one=[]; depth_three=[]
        accepted_depth_one={
            "OWNED_NATIVE_STRUCTURAL_SEGMENT_B2_RECURSIVE_COMPOSITION_STATE",
            "OWNED_NATIVE_STRUCTURAL_SEGMENT_BOUNDED_RECURSIVE_COMPOSITION_STATE",
        }
        for pos,row in enumerate(rows):
            ...
            if kind in accepted_depth_one:
                ...
                carrier=self._native_depth_three_structural_segment_parent_child_carrier(row,rows=rows,boot=boot)
                if carrier.get("status")!="CURRENT_DEPTH_THREE_STRUCTURAL_SEGMENT_PARENT_CHILD":
                    return {**base,**carrier,"status":"DEFER_UNKNOWN"}
                carrier["evidence_list_position"]=pos;depth_three.append(carrier)
            elif kind=="OWNED_NATIVE_STRUCTURAL_SEGMENT_DEPTH_FOUR_RECURSIVE_COMPOSITION_STATE":
                current=self._validate_current_native_structural_segment_depth_four_recursive_composition_state(row,rows=rows,boot=boot)
                if current.get("status")!="CURRENT_NATIVE_STRUCTURAL_SEGMENT_DEPTH_FOUR_RECURSIVE_COMPOSITION_STATE":
                    return {**base,**current,"status":"DEFER_UNKNOWN"}
        if not depth_three:
            return {**base,"status":"DEFER_UNKNOWN","reason":"CURRENT_DEPTH_THREE_SEGMENT_PARENT_REQUIRED"}
        d3=depth_three[-1]
        ancestry=set(str(x) for x in d3["full_ancestry_content_digests"]);ancestry.add(str(d3["composition_content_digest_sha256"]))
        ...
        child_digests=tuple(str(c["composition_content_digest_sha256"]) for c in children)
        child_depths=tuple(int(c["composition_depth"]) for c in children)
        content={"operator":"RECURSIVE_ORDERED_EVIDENCE_TUPLE","ordered_child_composition_content_digests":list(child_digests),"ordered_child_composition_depths":list(child_depths),"composition_depth":4,"child_arity":2,"identity_scope":"EXACT_GROUPED_OPERATIONAL_COMPOSITION_ONLY"}
        digest=action_result_digest(content);payload_children=[]
        for ordinal,carrier in enumerate(children):
            if int(carrier["composition_depth"])==3:
                payload_children.append({
                    "ordinal":ordinal,"source_parent_kind":"OWNED_NATIVE_STRUCTURAL_SEGMENT_DEPTH_THREE_RECURSIVE_COMPOSITION_STATE",
                    "source_parent_evidence_ref":carrier["source_parent_evidence_ref"],
                    "composition_content_digest_sha256":carrier["composition_content_digest_sha256"],
                    "composition_depth":3,"child_arity":2,
                    "nested_child_content_digests":list(carrier["nested_child_content_digests"]),
                ...
                payload_children.append({
                    "ordinal":ordinal,"source_parent_kind":carrier["source_parent_kind"],"source_parent_evidence_ref":carrier["source_parent_evidence_ref"],
                    "source_segment_state_evidence_ref":carrier["source_segment_state_evidence_ref"],"source_boundary_evidence_ref":carrier["source_boundary_evidence_ref"],
                    "composition_content_digest_sha256":carrier["composition_content_digest_sha256"],"composition_depth":1,"child_arity":2,
                    "nested_child_content_digests":list(carrier["nested_child_content_digests"]),"retrospective_provenance":"CURRENT_APPEND_ONLY_DEPTH_ONE_SEGMENT_PARENT_STATE",
                    "historical_event_authority":"NONE","flattening_authority":"NONE","associativity_authority":"NONE","authority_gain":"NONE",
                })
        payload={
            "kind":"OWNED_NATIVE_STRUCTURAL_SEGMENT_DEPTH_FOUR_RECURSIVE_COMPOSITION_STATE","runtime_boot_seq":boot,
            "operator_owner":"MICROSEED_NATIVE_STRUCTURAL_SEGMENT_DEPTH_FOUR_RECURSIVE_COMPOSITION",
            "composition_content_digest_sha256":digest,"composition_operator":"RECURSIVE_ORDERED_EVIDENCE_TUPLE",
            "max_input_parent_depth":3,"composition_depth":4,"child_arity":2,
            "ordered_child_composition_content_digests":list(child_digests),"ordered_child_composition_depths":list(child_depths),"children":payload_children,
            "selection_basis":base["selection_basis"],"grouping_basis":base["grouping_basis"],"identity_scope":"EXACT_GROUPED_OPERATIONAL_COMPOSITION_ONLY",
            "temporality":"CURRENT_RETROSPECTIVE_DERIVATION_APPENDED_AFTER_DEPTH_THREE_AND_EXTERNAL_DEPTH_ONE_PARENT_STATES",
            "generic_recursive_closure_authority":"NONE","depth_five_authority":"NONE","historical_event_authority":"NONE","ledger_rewrite_authority":"NONE",
            "flattening_authority":"NONE","associativity_authority":"NONE","semantic_composition_authority":"NONE","grammar_authority":"NONE",
            "effect_authority":"NONE","execution_authority":"NONE","scheduler_authority":"NONE","authority_gain":"NONE",
            "caller_supplied_parent_ids":"NO","caller_supplied_parent_order":"NO","caller_supplied_grouping":"NO","caller_supplied_depth":"NO","caller_supplied_output_evidence_id":"NO",
        }
        evidence_id="E-NATIVE-STRUCTURAL-SEGMENT-DEPTH-FOUR-RECURSIVE-"+action_result_digest(payload)[:24]
        existing=self.evidence.get(evidence_id)
        if existing is None:
            ref=self.append_evidence(evidence_id,payload,EpistemicStatus.PRESSURE_SUPPORTED,source="MICROSEED-NATIVE-STRUCTURAL-SEGMENT-DEPTH-FOUR-RECURSIVE-COMPOSITION")
            evidence_sha=ref.sha256;record_status="SEGMENT_DEPTH_FOUR_STATE_RECORDED"
        else:
            if existing.get("negative") or existing.get("payload")!=payload:
                return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_DEPTH_FOUR_STATE_EVIDENCE_ID_COLLISION","composition_state_evidence_id":evidence_id}
            evidence_sha=str(existing.get("sha256",""));record_status="SEGMENT_DEPTH_FOUR_STATE_ALREADY_PRESENT"
        return {**base,"status":"CURRENT_NATIVE_STRUCTURAL_SEGMENT_DEPTH_FOUR_RECURSIVE_COMPOSITION_STATE_RECORDED","composition_state_record_status":record_status,"composition_state_evidence_id":evidence_id,"composition_state_evidence_sha256":evidence_sha,"composition_content_digest_sha256":digest,"ordered_child_composition_content_digests":child_digests,"ordered_child_composition_depths":child_depths,"children":children,"caller_supplied_parent_ids":"NO","caller_supplied_parent_order":"NO","caller_supplied_grouping":"NO","caller_supplied_depth":"NO","caller_supplied_output_evidence_id":"NO"}
    ...
    def _native_depth_four_structural_segment_parent_child_carrier(
        ...
        for child in current.get("children",()):
            ancestry.add(str(child.get("composition_content_digest_sha256","")))
            for d in child.get("nested_child_content_digests",()): ancestry.add(str(d))
            for d in child.get("full_ancestry_content_digests",()): ancestry.add(str(d))
        ancestry.discard("")
        return {
            "status":"CURRENT_DEPTH_FOUR_STRUCTURAL_SEGMENT_PARENT_CHILD",
            "source_parent_kind":"OWNED_NATIVE_STRUCTURAL_SEGMENT_DEPTH_FOUR_RECURSIVE_COMPOSITION_STATE",
            "source_parent_evidence_ref":[str(row["evidence_id"]),str(row["sha256"])],
            "composition_content_digest_sha256":str(current["composition_content_digest_sha256"]),
            "composition_depth":4,"child_arity":2,
            "nested_child_content_digests":tuple(str(x) for x in current["ordered_child_composition_content_digests"]),
    ...
    def _validate_current_native_structural_segment_depth_five_recursive_composition_state(
        ...
        base={"max_input_parent_depth":4,"composition_depth":5,"child_arity":2,"recursive_depth_limit":5,
              "generic_recursive_closure_authority":"NONE","depth_six_authority":"NONE","historical_event_authority":"NONE",
              "ledger_rewrite_authority":"NONE","flattening_authority":"NONE","associativity_authority":"NONE",
              "semantic_composition_authority":"NONE","grammar_authority":"NONE","effect_authority":"NONE",
              "execution_authority":"NONE","scheduler_authority":"NONE","authority_gain":"NONE"}
        if row.get("negative"): return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_DEPTH_FIVE_STATE_MUST_BE_POSITIVE"}
        payload=row.get("payload") or {}
        if payload.get("kind")!="OWNED_NATIVE_STRUCTURAL_SEGMENT_DEPTH_FIVE_RECURSIVE_COMPOSITION_STATE": return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_DEPTH_FIVE_STATE_KIND_REQUIRED"}
        if int(payload.get("runtime_boot_seq",-1))!=boot: return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_DEPTH_FIVE_STATE_CURRENT_BOOT_REQUIRED"}
        if (payload.get("operator_owner")!="MICROSEED_NATIVE_STRUCTURAL_SEGMENT_DEPTH_FIVE_RECURSIVE_COMPOSITION"
            or payload.get("composition_operator")!="RECURSIVE_ORDERED_EVIDENCE_TUPLE"
            or int(payload.get("max_input_parent_depth",-1))!=4 or int(payload.get("composition_depth",-1))!=5
            or int(payload.get("child_arity",-1))!=2
            or payload.get("selection_basis")!="LATEST_CURRENT_DEPTH_FOUR_PARENT_PLUS_LATEST_CURRENT_EXTERNAL_DEPTH_ONE_PARENT_IN_EVIDENCE_ORDER"
            or payload.get("grouping_basis")!="WHOLE_DEPTH_FOUR_AND_DEPTH_ONE_PARENT_BOUNDARIES_PRESERVED"
            or payload.get("temporality")!="CURRENT_RETROSPECTIVE_DERIVATION_APPENDED_AFTER_DEPTH_FOUR_AND_EXTERNAL_DEPTH_ONE_PARENT_STATES"):
            return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_DEPTH_FIVE_STATE_OWNER_OR_SHAPE_MISMATCH"}
        for k in ("historical_event_authority","ledger_rewrite_authority","flattening_authority","associativity_authority",
                  "semantic_composition_authority","grammar_authority","effect_authority","execution_authority",
                  "scheduler_authority","authority_gain","generic_recursive_closure_authority","depth_six_authority"):
            if payload.get(k)!="NONE": return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_DEPTH_FIVE_STATE_AUTHORITY_OVERCLAIM","field":k}
        expected_id="E-NATIVE-STRUCTURAL-SEGMENT-DEPTH-FIVE-RECURSIVE-"+action_result_digest(payload)[:24]
        if str(row.get("evidence_id",""))!=expected_id: return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_DEPTH_FIVE_STATE_EVIDENCE_ID_NOT_DERIVED_FROM_CONTENT","expected_evidence_id":expected_id}
        pcs=tuple(payload.get("children",())); digs=tuple(str(x) for x in payload.get("ordered_child_composition_content_digests",())); depths=tuple(int(x) for x in payload.get("ordered_child_composition_depths",()))
        if len(pcs)!=2 or len(digs)!=2 or len(set(digs))!=2 or len(depths)!=2 or sorted(depths)!=[1,4]:
            return {**base,"status":"DEFER_UNKNOWN","reason":"EXACT_ONE_DEPTH_FOUR_AND_ONE_DEPTH_ONE_CHILD_REQUIRED"}
        positions={str(r.get("evidence_id","")):i for i,r in enumerate(rows)}; validated=[]; epos=[]; d4cur=None; d1cur=None
        for ordinal,(cp,digest,dep) in enumerate(zip(pcs,digs,depths)):
            if not isinstance(cp,dict) or int(cp.get("ordinal",-1))!=ordinal or int(cp.get("composition_depth",-1))!=dep:
                return {**base,"status":"DEFER_UNKNOWN","reason":"DEPTH_FIVE_CHILD_ORDINAL_OR_DEPTH_MISMATCH"}
            ref=tuple(cp.get("source_parent_evidence_ref",()))
            if len(ref)!=2: return {**base,"status":"DEFER_UNKNOWN","reason":"DEPTH_FIVE_CHILD_PARENT_REF_REQUIRED"}
            prow=self.evidence.get(str(ref[0]))
            if prow is None or str(prow.get("sha256",""))!=str(ref[1]): return {**base,"status":"DEFER_UNKNOWN","reason":"DEPTH_FIVE_CHILD_PARENT_REF_NOT_EXACT"}
            epos.append(int(positions.get(str(ref[0]),-1)))
            if dep==4:
                c=self._native_depth_four_structural_segment_parent_child_carrier(prow,rows=rows,boot=boot)
                if c.get("status")!="CURRENT_DEPTH_FOUR_STRUCTURAL_SEGMENT_PARENT_CHILD": return {**base,**c,"status":"DEFER_UNKNOWN"}
                expected={"ordinal":ordinal,"source_parent_kind":"OWNED_NATIVE_STRUCTURAL_SEGMENT_DEPTH_FOUR_RECURSIVE_COMPOSITION_STATE","source_parent_evidence_ref":c["source_parent_evidence_ref"],"composition_content_digest_sha256":c["composition_content_digest_sha256"],"composition_depth":4,"child_arity":2,"nested_child_content_digests":list(c["nested_child_content_digests"]),"full_ancestry_content_digests":list(c["full_ancestry_content_digests"]),"retrospective_provenance":"CURRENT_APPEND_ONLY_DEPTH_FOUR_SEGMENT_PARENT_STATE","historical_event_authority":"NONE","flattening_authority":"NONE","associativity_authority":"NONE","authority_gain":"NONE"};d4cur=c
            else:
                c=self._native_depth_one_structural_segment_parent_child_carrier(prow,rows=rows,boot=boot)
                if c.get("status")!="CURRENT_DEPTH_ONE_STRUCTURAL_SEGMENT_PARENT_CHILD": return {**base,**c,"status":"DEFER_UNKNOWN"}
                expected={"ordinal":ordinal,"source_parent_kind":c["source_parent_kind"],"source_parent_evidence_ref":c["source_parent_evidence_ref"],"source_segment_state_evidence_ref":c["source_segment_state_evidence_ref"],"source_boundary_evidence_ref":c["source_boundary_evidence_ref"],"composition_content_digest_sha256":c["composition_content_digest_sha256"],"composition_depth":1,"child_arity":2,"nested_child_content_digests":list(c["nested_child_content_digests"]),"retrospective_provenance":"CURRENT_APPEND_ONLY_DEPTH_ONE_SEGMENT_PARENT_STATE","historical_event_authority":"NONE","flattening_authority":"NONE","associativity_authority":"NONE","authority_gain":"NONE"};d1cur=c
            if cp!=expected or str(c["composition_content_digest_sha256"])!=digest: return {**base,"status":"DEFER_UNKNOWN","reason":"DEPTH_FIVE_CHILD_CONTENT_OR_PROVENANCE_MISMATCH"}
            validated.append(c)
        if any(x<0 for x in epos) or epos!=sorted(epos): return {**base,"status":"DEFER_UNKNOWN","reason":"DEPTH_FIVE_CHILD_ORDER_NOT_EVIDENCE_OWNED"}
        if d4cur is None or d1cur is None: return {**base,"status":"DEFER_UNKNOWN","reason":"EXACT_ONE_DEPTH_FOUR_AND_ONE_DEPTH_ONE_CHILD_REQUIRED"}
        ancestry=set(str(x) for x in d4cur["full_ancestry_content_digests"]);ancestry.add(str(d4cur["composition_content_digest_sha256"]))
        if str(d1cur["composition_content_digest_sha256"]) in ancestry: return {**base,"status":"DEFER_UNKNOWN","reason":"DEPTH_FIVE_EXTERNAL_DEPTH_ONE_PARENT_MUST_BE_OUTSIDE_FULL_DEPTH_FOUR_ANCESTRY"}
        content={"operator":"RECURSIVE_ORDERED_EVIDENCE_TUPLE","ordered_child_composition_content_digests":list(digs),"ordered_child_composition_depths":list(depths),"composition_depth":5,"child_arity":2,"identity_scope":"EXACT_GROUPED_OPERATIONAL_COMPOSITION_ONLY"}
        digest=action_result_digest(content)
        if str(payload.get("composition_content_digest_sha256",""))!=digest: return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_DEPTH_FIVE_STATE_CONTENT_DIGEST_MISMATCH"}
        return {**base,"status":"CURRENT_NATIVE_STRUCTURAL_SEGMENT_DEPTH_FIVE_RECURSIVE_COMPOSITION_STATE","composition_state_evidence_id":str(row["evidence_id"]),"composition_state_evidence_sha256":str(row["sha256"]),"composition_content_digest_sha256":digest,"ordered_child_composition_content_digests":digs,"ordered_child_composition_depths":depths,"children":tuple(validated),"selection_basis":payload["selection_basis"],"grouping_basis":payload["grouping_basis"],"temporality":payload["temporality"]}
    ...
    def derive_and_record_current_native_structural_segment_depth_five_recursive_composition(self, *, max_records: int = 4096) -> dict[str, Any]:
        base={"selection_basis":"LATEST_CURRENT_DEPTH_FOUR_PARENT_PLUS_LATEST_CURRENT_EXTERNAL_DEPTH_ONE_PARENT_IN_EVIDENCE_ORDER","grouping_basis":"WHOLE_DEPTH_FOUR_AND_DEPTH_ONE_PARENT_BOUNDARIES_PRESERVED","max_input_parent_depth":4,"composition_depth":5,"child_arity":2,"recursive_depth_limit":5,"generic_recursive_closure_authority":"NONE","depth_six_authority":"NONE","historical_event_authority":"NONE","ledger_rewrite_authority":"NONE","flattening_authority":"NONE","associativity_authority":"NONE","semantic_composition_authority":"NONE","grammar_authority":"NONE","effect_authority":"NONE","execution_authority":"NONE","scheduler_authority":"NONE","authority_gain":"NONE"}
        if max_records<=0:return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_DEPTH_FIVE_EVIDENCE_SCAN_BUDGET_REQUIRED"}
        total=self.evidence.count()
        ...
        rows=self.evidence.list();d1s=[];d4s=[];accepted_depth_one={"OWNED_NATIVE_STRUCTURAL_SEGMENT_B2_RECURSIVE_COMPOSITION_STATE","OWNED_NATIVE_STRUCTURAL_SEGMENT_BOUNDED_RECURSIVE_COMPOSITION_STATE"}
        for pos,row in enumerate(rows):
            payload=row.get("payload") or {};kind=payload.get("kind")
            if int(payload.get("runtime_boot_seq",-1))!=boot:continue
            if kind in accepted_depth_one:
                c=self._native_depth_one_structural_segment_parent_child_carrier(row,rows=rows,boot=boot)
                if c.get("status")!="CURRENT_DEPTH_ONE_STRUCTURAL_SEGMENT_PARENT_CHILD":return {**base,**c,"status":"DEFER_UNKNOWN"}
                c["evidence_list_position"]=pos;d1s.append(c)
            elif kind=="OWNED_NATIVE_STRUCTURAL_SEGMENT_DEPTH_FOUR_RECURSIVE_COMPOSITION_STATE":
                c=self._native_depth_four_structural_segment_parent_child_carrier(row,rows=rows,boot=boot)
                if c.get("status")!="CURRENT_DEPTH_FOUR_STRUCTURAL_SEGMENT_PARENT_CHILD":return {**base,**c,"status":"DEFER_UNKNOWN"}
                c["evidence_list_position"]=pos;d4s.append(c)
            elif kind=="OWNED_NATIVE_STRUCTURAL_SEGMENT_DEPTH_FIVE_RECURSIVE_COMPOSITION_STATE":
                cur=self._validate_current_native_structural_segment_depth_five_recursive_composition_state(row,rows=rows,boot=boot)
                if cur.get("status")!="CURRENT_NATIVE_STRUCTURAL_SEGMENT_DEPTH_FIVE_RECURSIVE_COMPOSITION_STATE":return {**base,**cur,"status":"DEFER_UNKNOWN"}
        if not d4s:return {**base,"status":"DEFER_UNKNOWN","reason":"CURRENT_DEPTH_FOUR_SEGMENT_PARENT_REQUIRED"}
        d4=d4s[-1];ancestry=set(str(x) for x in d4["full_ancestry_content_digests"]);ancestry.add(str(d4["composition_content_digest_sha256"]))
        external=[c for c in d1s if str(c["composition_content_digest_sha256"]) not in ancestry]
        ...
        d1=external[-1];children=tuple(sorted((d4,d1),key=lambda x:int(x["evidence_list_position"])));digs=tuple(str(c["composition_content_digest_sha256"]) for c in children);depths=tuple(int(c["composition_depth"]) for c in children)
        content={"operator":"RECURSIVE_ORDERED_EVIDENCE_TUPLE","ordered_child_composition_content_digests":list(digs),"ordered_child_composition_depths":list(depths),"composition_depth":5,"child_arity":2,"identity_scope":"EXACT_GROUPED_OPERATIONAL_COMPOSITION_ONLY"};digest=action_result_digest(content);pcs=[]
        for ordinal,c in enumerate(children):
            if int(c["composition_depth"])==4: pcs.append({"ordinal":ordinal,"source_parent_kind":"OWNED_NATIVE_STRUCTURAL_SEGMENT_DEPTH_FOUR_RECURSIVE_COMPOSITION_STATE","source_parent_evidence_ref":c["source_parent_evidence_ref"],"composition_content_digest_sha256":c["composition_content_digest_sha256"],"composition_depth":4,"child_arity":2,"nested_child_content_digests":list(c["nested_child_content_digests"]),"full_ancestry_content_digests":list(c["full_ancestry_content_digests"]),"retrospective_provenance":"CURRENT_APPEND_ONLY_DEPTH_FOUR_SEGMENT_PARENT_STATE","historical_event_authority":"NONE","flattening_authority":"NONE","associativity_authority":"NONE","authority_gain":"NONE"})
            else: pcs.append({"ordinal":ordinal,"source_parent_kind":c["source_parent_kind"],"source_parent_evidence_ref":c["source_parent_evidence_ref"],"source_segment_state_evidence_ref":c["source_segment_state_evidence_ref"],"source_boundary_evidence_ref":c["source_boundary_evidence_ref"],"composition_content_digest_sha256":c["composition_content_digest_sha256"],"composition_depth":1,"child_arity":2,"nested_child_content_digests":list(c["nested_child_content_digests"]),"retrospective_provenance":"CURRENT_APPEND_ONLY_DEPTH_ONE_SEGMENT_PARENT_STATE","historical_event_authority":"NONE","flattening_authority":"NONE","associativity_authority":"NONE","authority_gain":"NONE"})
        payload={"kind":"OWNED_NATIVE_STRUCTURAL_SEGMENT_DEPTH_FIVE_RECURSIVE_COMPOSITION_STATE","runtime_boot_seq":boot,"operator_owner":"MICROSEED_NATIVE_STRUCTURAL_SEGMENT_DEPTH_FIVE_RECURSIVE_COMPOSITION","composition_content_digest_sha256":digest,"composition_operator":"RECURSIVE_ORDERED_EVIDENCE_TUPLE","max_input_parent_depth":4,"composition_depth":5,"child_arity":2,"ordered_child_composition_content_digests":list(digs),"ordered_child_composition_depths":list(depths),"children":pcs,"selection_basis":base["selection_basis"],"grouping_basis":base["grouping_basis"],"identity_scope":"EXACT_GROUPED_OPERATIONAL_COMPOSITION_ONLY","temporality":"CURRENT_RETROSPECTIVE_DERIVATION_APPENDED_AFTER_DEPTH_FOUR_AND_EXTERNAL_DEPTH_ONE_PARENT_STATES","generic_recursive_closure_authority":"NONE","depth_six_authority":"NONE","historical_event_authority":"NONE","ledger_rewrite_authority":"NONE","flattening_authority":"NONE","associativity_authority":"NONE","semantic_composition_authority":"NONE","grammar_authority":"NONE","effect_authority":"NONE","execution_authority":"NONE","scheduler_authority":"NONE","authority_gain":"NONE","caller_supplied_parent_ids":"NO","caller_supplied_parent_order":"NO","caller_supplied_grouping":"NO","caller_supplied_depth":"NO","caller_supplied_output_evidence_id":"NO"}
        evidence_id="E-NATIVE-STRUCTURAL-SEGMENT-DEPTH-FIVE-RECURSIVE-"+action_result_digest(payload)[:24];existing=self.evidence.get(evidence_id)
        if existing is None:
            ref=self.append_evidence(evidence_id,payload,EpistemicStatus.PRESSURE_SUPPORTED,source="MICROSEED-NATIVE-STRUCTURAL-SEGMENT-DEPTH-FIVE-RECURSIVE-COMPOSITION");esha=ref.sha256;rstatus="SEGMENT_DEPTH_FIVE_STATE_RECORDED"
        else:
            if existing.get("negative") or existing.get("payload")!=payload:return {**base,"status":"DEFER_UNKNOWN","reason":"SEGMENT_DEPTH_FIVE_STATE_EVIDENCE_ID_COLLISION","composition_state_evidence_id":evidence_id}
            esha=str(existing.get("sha256",""));rstatus="SEGMENT_DEPTH_FIVE_STATE_ALREADY_PRESENT"
        return {**base,"status":"CURRENT_NATIVE_STRUCTURAL_SEGMENT_DEPTH_FIVE_RECURSIVE_COMPOSITION_STATE_RECORDED","composition_state_record_status":rstatus,"composition_state_evidence_id":evidence_id,"composition_state_evidence_sha256":esha,"composition_content_digest_sha256":digest,"ordered_child_composition_content_digests":digs,"ordered_child_composition_depths":depths,"children":children,"caller_supplied_parent_ids":"NO","caller_supplied_parent_order":"NO","caller_supplied_grouping":"NO","caller_supplied_depth":"NO","caller_supplied_output_evidence_id":"NO"}
