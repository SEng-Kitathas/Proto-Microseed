...
def _control_boundary(m,tag,state='S-STORE-EQUIV'):
    out=m.observe_opaque_control_state(
        Observation(f'CAP-{tag}','EXTERNAL','opaque-control',state,authority=Authority.OBSERVATION_ONLY),
        evidence_id=f'E-CONTROL-{tag}',
    )
...
def test_arbitrary_non_token_evidence_is_grouping_neutral_and_prototype_matches_production():
    td,m,world,seeded=fixture4(tokens=('R4','T9','W3','K7'))
    try:
        _control_boundary(m,'EVIDENCE-NEUTRAL-START')
        _obs(m,('R4','T9'),'STORE-EQUIV-EVIDENCE-PRE',22300)
        m.append_evidence('E-STORE-EQUIV-NONBOUNDARY',{'kind':'REPRESENTED_NON_TOKEN_BOUNDARY_TEST','runtime_boot_seq':m._current_runtime_boot_seq()},EpistemicStatus.PRESSURE_SUPPORTED,source='HOSTILE-WORLD')
        ...
        assert proto['derived_arity']==prod['derived_arity']==4
        assert proto['composition_content_digest_sha256']==prod['composition_content_digest_sha256']
        assert proto['last_boundary']['kind']=='OPAQUE_CONTROL_STATE_OBSERVED'
        assert proto['last_boundary']['evidence_id']=='E-CONTROL-EVIDENCE-NEUTRAL-START'
        assert proto['caller_evidence_grouping_authority']=='NONE'
        assert prod['last_window_boundary']['kind']=='OPAQUE_CONTROL_STATE_OBSERVED'
    finally:close4(m);td.cleanup()
