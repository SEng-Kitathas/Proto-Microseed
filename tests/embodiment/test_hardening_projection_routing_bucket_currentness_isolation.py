...
def test_bad_bucket_cannot_be_masked_by_good_outcomes_from_same_relation_in_other_qualified_bucket():
    ...
    try:
        ...
        good_rows=ms._projection_conditioned_post_binding_relation_experiences(binding,fx['bucket1'],rel)
        bad_rows=ms._projection_conditioned_post_binding_relation_experiences(binding,fx['bucket0'],rel)
        good_w=assess_action_outcome_predictive_currentness(rel,good_rows,PredictiveCurrentnessConfig())
        bad_w=assess_action_outcome_predictive_currentness(rel,bad_rows,PredictiveCurrentnessConfig())
        assert len(good_rows)==48 and len(bad_rows)==16
        assert good_w.window_accuracies==(1.0,)*6 and good_w.status=='CURRENT_WITHIN_BOUNDS'
        assert bad_w.window_accuracies==(0.0,0.0) and bad_w.status=='DRIFT_WITNESS'
        assert set(x.evidence_id for x in bad_rows)==set(bad_eids)
...
def test_exact_routing_and_bucket_ancestry_survives_rehearsal_persistence_restart():
    m=_m();fx=m.build_integrated();ms=fx['ms'];root=Path(fx['td'].name)
    ms2=None
    try:
        cid,rid,rel,bid=_same_relation_two_bucket_binding(m,fx)
        for i,(raw,bucket) in enumerate(((('R0','H0','C0','RM0'),fx['bucket0']),(('R1','H0','C1','RM1'),fx['bucket1']))):
            p,out=_routed_episode(m,fx,bid,cid,raw,f'PERSIST-{i}',0.00001*i,rel.value_effect,rel.next_state_id)
            assert p.projection_routing_id==bid
            assert p.projection_bucket_id==bucket
            packet=p.serializable()
            assert packet['projection_routing_id']==bid and packet['projection_bucket_id']==bucket
        expected={(p.proposal_id,p.projection_bucket_id) for p in ms.counterfactual_rehearsals.proposals.values() if p.projection_routing_id==bid}
        assert {b for _,b in expected}=={fx['bucket0'],fx['bucket1']}
        ms2=m.Microseed(root)
        loaded={(p.proposal_id,p.projection_bucket_id) for p in ms2.counterfactual_rehearsals.proposals.values() if p.projection_routing_id==bid}
        assert loaded==expected
    finally:
        if ms2 is not None:
            try:ms2.biography.close();ms2.evidence.conn.close();ms2.store.conn.close()
            except Exception:pass
        try:ms.biography.close();ms.evidence.conn.close();ms.store.conn.close()
        finally:fx['td'].cleanup()


def test_ambiguous_legacy_routed_descendant_without_bucket_ancestry_fails_binding_closed():
    m=_m();fx=m.build_integrated();ms=fx['ms']
    try:
        cid,rid,rel,bid=_same_relation_two_bucket_binding(m,fx)
        raw=('LEG','H0','C1','LEGM')
        p,_=_routed_episode(m,fx,bid,cid,raw,'LEGACY-SIM',0.0,rel.value_effect,rel.next_state_id)
        assert p.projection_routing_id==bid and p.projection_bucket_id==fx['bucket1']
        # Simulate a persisted pre-V2 routed proposal whose relation/binding evidence
        # exists but whose exact selected bucket was never carried durably.
        legacy=replace(p,projection_routing_id=None,projection_bucket_id=None)
        ms.counterfactual_rehearsals.proposals[p.proposal_id]=legacy
        assert ms.projection_conditioned_relation_routing_status(bid)['status']=='STALE_PROJECTION_CONDITIONED_ROUTING'
    finally:
        try:ms.biography.close();ms.evidence.conn.close();ms.store.conn.close()
        finally:fx['td'].cleanup()
