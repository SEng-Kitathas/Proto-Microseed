...
def install(m):
    m.register_capability(CapabilityContract('OBS','obs',{}, {},(),(),Authority.OBSERVATION_ONLY,('MS1752',),'CURRENT',{},query_obligation_id='OBS-Q',qualification=QualificationState.SHADOW_QUALIFIED,handler=lambda **_:dict(PAYLOAD),operational_scope_id='R2'))
    m.register_capability(CapabilityContract('BASIS','basis',{}, {},(),(),Authority.DERIVED_READ_ONLY,('MS1752',),'CURRENT',{},dependencies=('OBS',),query_obligation_id='BASIS-Q',qualification=QualificationState.SHADOW_QUALIFIED,handler=lambda **_:{'claim':'LIVE'},operational_scope_id='R2'))
...
def test_same_frame_id_epoch_with_changed_frame_content_is_rejected_after_pass06_repair():
    with tempfile.TemporaryDirectory(prefix='ms1752-') as td:
        ...
        # Hostile reincarnation/content alias: same frame handle and epoch, altered content signature.
        m.frames.frames['F'].signature_sha256='2'*64
        assert m.frames.is_current('F',0)
        r=m.derive_admitted_opaque_transition_sample(eid)
        assert r=={'status':'ABSTAIN','reason':'OPERATIONAL_FRAME_CONTENT_DRIFT','authority':'NONE'}
